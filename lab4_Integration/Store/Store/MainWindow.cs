...
namespace Store
{
    public partial class MainWindow : Form
    {
        ...
        //Triggers as the user changes between Register and Storage. Clears fields of old data.
        private void tabControl1_SelectedIndexChanged(object sender, EventArgs e)
        {
            sub_registerTab.SelectedTab = sell_tab;
            clearAllFields();
        }
        ...
        //Creates a messageBox with instructions of how to use the "sell"-tab.
        ...
        //Handles the import and export of register. 
        private void export_import_button_Click(object sender, EventArgs e)
        {
            if (registerObject.isCartEmpty())
            {
                if (((Button)sender).Name == "export_button")
                {
                    if (storageObject.saveStorage(true))
                        MessageBox.Show("Successfully exported the register!");
                    else
                        MessageBox.Show("Export of products failed. Try again later.");
                }
                else
                {
                    if (storageObject.loadStorage(true))
                    {
                        update_All_Lists();
                        MessageBox.Show("Successfully imported the register!");
                    }
                    else
                        MessageBox.Show("Import of products failed. Try again later.");
                }
            }
            else
                MessageBox.Show("Cart is not empty. Try again when cart is empty.");
        }
        ...
        {
            int ID = 0;
            List<Product> temp_list = storageObject.getAvailableProducts();
            foreach(Product x in temp_list)
            {
                if (x.P_ID > ID)
                    ID = x.P_ID;
            }
